...
To be honest, this is intrinsically just doing an interpolation for the distribution at one exact time. So it basically just an demo, without pratical usability.

#### Result of Demo

![](assets/Demo_Result.png)
...
# uv (Recommended)
uv init
uv add pytorch numpy matplotlib scipy deepxde
uv run **.py

# pip
python3 -m venv venv
pip install pytorch numpy matplotlib scipy deepxde
python3 **.py
```